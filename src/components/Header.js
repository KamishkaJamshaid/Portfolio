...
const Header = () => {
  ...
  return (
    <div className="flex flex-wrap items-center justify-between sticky top-0 z-50 bg-violet-950 p-4 lg:p-7 h-20 lg:h-28">
      ...
      <header className="flex flex-1 justify-center items-center">
        <div className="lg:hidden relative">
          <button onClick={toggleMenu} className="text-white">
            {isMenuOpen ? <FaTimes size={24} className=' md:w-10 md:h-7'/> : <FaBars size={24} className=' md:w-10 md:h-7' />}
          </button>
        </div>

        <nav className={`${
            isMenuOpen ? 'flex' : 'hidden'
          } lg:flex flex-col lg:flex-row lg:items-center justify-center bg-violet-950 lg:bg-transparent absolute lg:static border-2 lg:no-underline sm:underline lg:border-none top-full left-0 w-40 md:w-40 lg:w-auto space-y-2 lg:space-y-0 lg:space-x-4 mt-2 lg:mt-0 p-2 lg:p-0 text-white rounded-lg md:mx-56 mx-16`}
        >
